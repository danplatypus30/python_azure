...
async def retrieve_vt_analysis_stats(url):
    """
    ...
    try:
        ...
        analysis_result = virustotal3.core.URL(api_key).info_url(url, timeout=2500)
...
async def check_domain_punycode(url):
    """
...
def download_blocklist():
    ...
    urls = ['https://zonefiles.io/f/compromised/domains/full/compromised_domains_full.txt']
    ...
    try:
        ...
        for url in urls:
            ...
            curr_domains_list.append(r.text)
        ...
        with open(filepath, 'w') as output_file:
            ...
            output_file.writelines(curr_domains_list)
    except:
        return None


async def check_url_in_blocklist(url):
    """
...
async def retrieve_whois(url, xml=None):
    """
...
async def req_vt_whois_maldom_puny(url):
    """
    ...
    try:
        # MalDomain.py
        results['maldomain_result'] = await check_url_in_blocklist(url)
        # WHOIS.py
        results['whois_result'] = await retrieve_whois(url)
        # VT.py
        ...
        results['vt_result'] = await retrieve_vt_analysis_stats(url)
        # PunycodeDet.py
        results['puny_result'] = await check_domain_punycode(url)
